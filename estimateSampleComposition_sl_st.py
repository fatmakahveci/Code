...
Options:
    ...
    -v <allele_st_info>                             VCF file containing alleles and strain incidence info.
...
def readStrainAlleles(strains_vcf, valid_allele_support, exclude_samples, ignore_ref_alleles=False):
    ...
    if strain_samples:  # inferred VCF; strain->samples encoded in ##SAMPLE=<ID=strain,GENOMES=s1;s2...> fields

        sample_strains = [next((st for st, st_s in strain_samples.items() if s in st_s), None) if s not in exclude_samples else None for s in vcf.header.samples]
        strain_samples = dict()
        ...
        strain_samples = {st: frozenset((si,)) for si, st in enumerate(sample_strains)}
    ...
    class AlleleOcc:
        ...
        @staticmethod
        def counts_to_frequencies(st_counts):
            return {st: float(stc) / len(AlleleOcc.s_strain_samples[st]) for st, stc in st_counts.items() if stc >= AlleleOcc.s_sig_sample_count[st]}
        ...
        def merge_update(self, samples):
            ...
            if extra:
                ...
                for s in extra:
                    ...
                    st = AlleleOcc.s_sample_strains[s]
                    self.strains[st] = self.strains.get(st, 0) + 1.0 / len(AlleleOcc.s_strain_samples[st])
        ...
        def merge_copy(self, samples):
            ...
            if extra:
                ...
                for s in extra:
                    st = AlleleOcc.s_sample_strains[s]
                    subl[st] = subl.get(st, 0) + 1.0 / len(AlleleOcc.s_strain_samples[st])
    ...
    st_allele_counts = {s: 0 for s in strain_samples.keys()}
    ignored_alleles = set()
    ...
    for variant in vcf.fetch():
        ...
        for sample_gt in variant.samples.itervalues():
            ...
            if allele_idx is not None and (not ignore_ref_alleles or allele_idx > 0):
                ...
                st = sample_strains[s_idx]

                if st is not None:

                    stc_a = allele_strain_counts[allele_idx]
                    stc_a[st] = stc_a.get(st, 0) + 1
        ...
        pos, vref, stc_r = variant.pos - 1, variant.ref, AlleleOcc.counts_to_frequencies(allele_strain_counts[0])

        if not AlleleOcc.is_discriminating(stc_r) or (pos, vref, pos + 1) in ignored_alleles:
            stc_r = None
        ...
        if len(vref) == 1 and all(len(va) == 1 for va in variant.alts):   # common case - all SNPs
            ...
            if stc_r:
                ...
                if ivl:
                    ...
                    allele_incidence.addi(pos, pos + 1, AlleleOcc(vref, stc_r, allele_occs[vref]))

                    for st in stc_r:
                        st_allele_counts[st] += 1  # not exact because in rare instances we discard some alleles

            for va, stc_a in zip(variant.alts, allele_strain_counts[1:]):

                if va not in allele_occs:
                    continue

                stc_a = AlleleOcc.counts_to_frequencies(stc_a)

                if AlleleOcc.is_discriminating(stc_a):

                    allele_incidence.addi(pos, pos + 1, AlleleOcc(va, stc_a, allele_occs[va]))
    ...
    return PopulationAllelesInfo(allele_incidence=allele_incidence_per_contig,
                                 ...
                                 strain_allele_counts=st_allele_counts)
...
# @profile
def GuesstimateInitialStrainPriors(bam_in, st_alleles, bed_regions, min_reads, min_contrib, st_cutoff):
        ...
        for _, rgn in intervals:
            ...
            if len(disc_allele.allele) == 1:
                ...
                if bc > n_mc:
                    ...
                    for cur_st, cur_st_f in disc_allele.strains.items():
                        n_c, s_c = st_alleles_contrib[cur_st]
                        st_alleles_contrib[cur_st] = (n_c + 1, s_c + bc * cur_st_f)  # coverage, P(allele)P(st|allele)
    ...
    st_alleles_contrib, allele_incidence = defaultdict(lambda: (0, 0)), st_alleles.allele_incidence
    ...
    if st_cutoff > 0:
        ...
        lc_ok = {st: ac[0] > st_cutoff * st_alleles.strain_allele_counts[st] for st, ac in st_alleles_contrib.items()}
        st_priors = {st: ac[0] if lc_ok[st] else 0.0 for st, ac in st_alleles_contrib.items()}

    else:
        st_priors = {st: ac[0] for st, ac in st_alleles_contrib.items()}

    logger.debug('st_priors before normalisation: {}'.format(st_priors))

    stp_norm = float(sum(st_priors.values()))

    if stp_norm > 0:
        st_priors = {l: v / stp_norm for l, v in st_priors.items()}

    logger.info("Initialised strain distribution: {}\n".format(present_distribution(st_priors)))
...
def ComputeHaplotypeLogProbabilities_approx(haplotype, all_alleles, strain_samples):
    ...
    if not hasattr(ComputeHaplotypeLogProbabilities_approx, 'st_probs_cache'):
        ComputeHaplotypeLogProbabilities_approx.st_probs_cache = odict()

    allele_set_str = '|'.join(tuple(str((a.begin, a.end, a.data.allele)) for a in haplotype))
    st_probs = ComputeHaplotypeLogProbabilities_approx.st_probs_cache.get(allele_set_str)

    if st_probs is not None:  # could be empty map though. hence explicit comparison with None
        return st_probs

    st_probs, supporting_samples = {}, frozenset(itertools.chain.from_iterable(a.data.samples for a in haplotype))
        ...
        for l, ls in strain_samples.items():
            ...
            if len(l_samples) >= min(int(args["--valid_allele_support"]), len(strain_samples[l])):  # ignore low evidence
                ...
                if unsupported_alleles_penalty >= error_values.err_limit_log_odds:
                    st_probs[l] = math.log1p(len(l_samples) / (len(all_l_samples) + 1.0e-8) - 1.0) + unsupported_alleles_penalty

    # OK to limit cache size because only proximate alleles will be in cache. BAM is traversed in order, so forget past.
    if len(ComputeHaplotypeLogProbabilities_approx.st_probs_cache) == 0x100000:

        for j in range(1024):  # evict older entries

            ComputeHaplotypeLogProbabilities_approx.st_probs_cache.popitem(False)

    ComputeHaplotypeLogProbabilities_approx.st_probs_cache[allele_set_str] = st_probs
...
# error rate to aggressively penalise smaller subsets
# @profile
def ComputeHaplotypeLogProbabilities(haplotype, all_alleles, strain_samples, len_penalty=0.0, seen_st_counts=None, seen_st_samples=None):

    if not hasattr(ComputeHaplotypeLogProbabilities, 'st_probs_cache'):

        ComputeHaplotypeLogProbabilities.st_probs_cache = odict()

    allele_set_str = '|'.join(tuple(str((a.begin, a.end, a.data.allele)) for a in haplotype))
    st_probs_x = ComputeHaplotypeLogProbabilities.st_probs_cache.get(allele_set_str)

    if st_probs_x is not None:  # could be empty map though. hence explicit comparison with None

        st_tuple = st_probs_x[1]

        if len(st_tuple) > 0 and seen_st_counts is not None:

            for l in st_tuple:

                seen_st_counts[l] = seen_st_counts.get(l, 0) + 1

        return {l: v + len_penalty for l, v in st_probs_x[0].items()}

    error_values = implied_errors_()

    # check the most common maximal subset for each strain, penalising subset score by length penalty
    st_probs, n_alleles = {}, len(haplotype)
    ...
    if n_alleles > 1 and len_penalty >= error_values.err_limit_log_odds:
        ...
        for j in range(n_alleles):  # n_alleles iterations, i.e. n!/(n-m)! complexity
            ...
            sub_st_prob = ComputeHaplotypeLogProbabilities(haplotype, all_alleles, strain_samples, sub_penalty, seen_subs, seen_sub_samples).items()

            for l, v in sub_st_prob:

                if v > st_probs.get(l, -1000):
                    st_probs[l] = v
    ...
    if len(ComputeHaplotypeLogProbabilities.st_probs_cache) == 0x100000:

        for j in range(1024):  # evict older entries
            ComputeHaplotypeLogProbabilities.st_probs_cache.popitem(False)

    st_probs_u = {l: v - len_penalty for l, v in st_probs.items()}    # cache unpenalised values
    ComputeHaplotypeLogProbabilities.st_probs_cache[allele_set_str] = (st_probs_u, tuple(seen_st))

    return st_probs


def ComputeStrainLikelihoodsForRead(r, st_alleles, approximate):

    contig = r.reference_name
    all_alleles = slist(st_alleles.allele_incidence[contig][r.reference_start:r.reference_end], key=lambda i: i.begin)
    ...
    have_incidence_info = any(len(sts) > 1 for sts in st_alleles.strain_samples.values())

    haplotype, alleles_list = slist(key=all_alleles.key), slist(key=all_alleles.key)
    total_alleles, alleles_per_st = 0, defaultdict(lambda: 0)
    ...
    if have_incidence_info:
        ...
        stra_samples = st_alleles.strain_samples
        st_likelihoods = ComputeHaplotypeLogProbabilities_approx(haplotype, alleles_list, stra_samples) if approximate else ComputeHaplotypeLogProbabilities(haplotype, alleles_list, stra_samples)
        ...
        if approximate:  # P(alleles|l) = 1 if all alleles belong to l, else 0
            st_likelihoods = {st: 0 for st, a_st in alleles_per_st.items() if a_st >= total_alleles}
            ...
            st_likelihoods = {l: errs.log_odds * (total_alleles - sc_l) for l, sc_l in alleles_per_st.items() if sc_l >= min_snp_count}

    logger.debug('st_likelihoods: {}'.format(st_likelihoods))

    if not st_likelihoods:

        raise InconsistentSupportException(r.query_name)

    max_rw = max(st_likelihoods.values())  # offset all values so maximum is computable (i.e. scale all likelihoods)
    offset = -(max_rw + 22) if max_rw < -22 else 0

    return {l: math.exp(st_likelihoods[l] + offset) for l in st_likelihoods.keys()}  # detected strains only


def ProcessSample(bam_in, regions, st_alleles, min_reads, approximate):
    ...
    for r in bam_in.fetch(*reads_range) if reads_range else bam_in.fetch():
        ...
        try:
            strain_likelihoods = ComputeStrainLikelihoodsForRead(r, st_alleles, approximate)
...
def EstimateStrainPriors(reads_info, regional_coverage, st_priors):

    n_rounds, weighted_reads = 0, [(r_l, r_p / regional_coverage[r_r]) for r_l, r_p, r_r, r_n in reads_info if regional_coverage[r_r] > 0 and any(st_priors.get(st, 0) > 1e-20 for st in r_l)]

    while any(lp > 0 for lp in st_priors.values()) and n_rounds < 2500:

        n_rounds += 1
        logger.debug('round {} - cur_st_prevs: {}\n'.format(n_rounds, st_priors))
        updated_priors = {l: 0.0 for l in strains}

        for r_likelihoods, r_p in weighted_reads:

            r_posteriors = {l: st_priors[l] * s for l, s in r_likelihoods.items() if st_priors[l] > 0}
        ...
        if any(abs(v - st_priors[l]) > float(args["--convergence_threshold"]) for l, v in updated_priors.items()):
            st_priors = updated_priors
...
def FilterEstimatedPriors(strain_priors, reads_info, regional_coverage, sig_level):
    ...
    for r_likelihoods, r_weight, r_rgn, r_name in t_info:
        ...
        if r_name != t_name:
            ...
            if max_p > 0:
                st_s = tuple(l for l, lp in strain_priors.items()
                             if lp >= sig_level and max_p - t_likelihoods.get(l, 0) * lp < 1e-12)
                if st_s:
                    st = st_s[0] if len(st_s) == 1 else choice(st_s)  # estimated read origin
                    strain_popularity[st] += t_weight
    ...
    kept_strains = {st: strain_priors[st] for st, w in strain_popularity.items() if w >= min_reads_weight}
    ...
    for l, p in strain_priors.items():
        ...
        if p < st:
            ...
            logger.warning("Discarding popular strain {} because {} < {}".format(l, p, st))
            strain_priors[l] = 0
            lp_sum -= p
...
def EstimateSampleCompositions(sampleBam, bedRegions, st_alleles, approx, sigLevel, minCoverage, minContrib, minLdsnps):
    ...
    with pysam.AlignmentFile(sampleBam, 'rb') as bam_in:
        ...
        if minContrib < 0:
            ...
            strain_priors = GuesstimateInitialStrainPriors(bam_in, st_alleles, bedRegions, minCoverage, minContrib, minLdsnps)
            strain_priors.update({l: 0.0 for l in strains if l not in strain_priors})

        reads_info, coverage = ProcessSample(bam_in, bedRegions, st_alleles, minCoverage, approx)
...
if __name__ == '__main__':
    ...
    st_alleles = readStrainAlleles(args["-v"], int(args["--valid_allele_support"]), x_samples)

    logger.info("Relevant alleles = {}".format(sum(map(len, st_alleles.allele_incidence.values()))))

    strains = frozenset(st_alleles.strain_samples.keys())
    assert strains == frozenset(st_alleles.strain_allele_counts.keys())

    bedRegions = readBedFile(args["-b"]) if args["-b"] else None

    def analyse_single_bam(bam_in):

        filtered_priors, reads_info = EstimateSampleCompositions(bam_in, bedRegions, st_alleles, args["-a"],
